...
Generated on 2026-07-13T07:28:42Z
Total commits: 60

## 1e659361e338139346a4ed87175be2ae890783a2 — 2026-07-10T12:44:17+05:30
...
## d7a516ab4a5d27c86965fe5c470d04cb1953ed71 — 2026-07-10T12:43:45+05:30

Message:

removing all unusable files for app

```diff
diff --git a/backend/app/main.py b/backend/app/main.py
index 5b71506..1b4e111 100644
...
+app = FastAPI(title="RAG Evaluation", version="1.0.0")
 ...
 app.add_middleware(
     CORSMiddleware,
@@ -71,162 +22,8 @@ app.add_middleware(
 )
 ...
 def _eval_accuracy_llm(system: str, prompt: str) -> str:
     if settings.eval_accuracy_provider == "ollama":
         payload = {
             ...
             "messages": [
@@ -244,9 +41,13 @@ def _eval_accuracy_llm(system: str, prompt: str) -> str:
                 timeout=60,
             )
...
+                return data["response"]
             ...
             if exc.response.status_code != 404:
                 raise
             resp = httpx.post(
@@ -259,17 +60,7 @@ def _eval_accuracy_llm(system: str, prompt: str) -> str:
                 },
                 timeout=60,
             )
+            resp.raise_for_status()
             ...
             if "response" in data:
                 return data["response"]
@@ -287,602 +78,31 @@ def _eval_accuracy_llm(system: str, prompt: str) -> str:
...
@@ -896,10 +116,7 @@ async def rag_evaluate(req: EvaluationRequest):
...
+            accuracy = _to_accuracy(acc_str)
             ...
             per_question.append(PerQuestionResult(
                 question=q.question,
@@ -922,26 +139,29 @@ async def rag_evaluate(req: EvaluationRequest):
...
@@ -959,133 +179,8 @@ async def rag_evaluate(req: EvaluationRequest):
...
index b5ce8f9..0000000
+++ /dev/null
@@ -1,176 +0,0 @@
diff --git a/backend/app/storage.py b/backend/app/storage.py
deleted file mode 100644
index b389690..0000000
+++ /dev/null
@@ -1,156 +0,0 @@
diff --git a/helpfull scripts/check_OCR.py b/helpfull scripts/check_OCR.py
deleted file mode 100644
index f5af9b2..0000000
+++ /dev/null
@@ -1,491 +0,0 @@
...
deleted file mode 100644
index c0b8e97..0000000
+++ /dev/null
@@ -1,141 +0,0 @@
diff --git a/helpfull scripts/upload_to_elasticsearch.py b/helpfull scripts/upload_to_elasticsearch.py
deleted file mode 100644
index 44c990e..0000000
+++ /dev/null
@@ -1,375 +0,0 @@
\ No newline at end of file
diff --git a/test_synonyms.py b/test_synonyms.py
deleted file mode 100644
index d8f8297..0000000
+++ /dev/null
@@ -1,96 +0,0 @@
```

## 79c5e2dd4889f8dd160e9cc8da2330332d3d86cf — 2026-07-10T12:24:08+05:30

Message:

live call and speak with AI agent

```diff
diff --git a/helpfull scripts/check_OCR.py b/helpfull scripts/check_OCR.py
new file mode 100644
index 0000000..f5af9b2
+++ b/helpfull scripts/check_OCR.py	
@@ -0,0 +1,491 @@
+import os
+import sys
+import json
...
## 962a0d5950163961efbdddea52bff250b74a7be2 — 2026-07-08T14:33:22+05:30

Message:

live user interaction with AI RAG

```diff
diff --git a/backend/app/chroma_store.py b/backend/app/chroma_store.py
index 1170b82..49f977b 100644
...
@@ -37,6 +42,16 @@ UPLOAD_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), "uploads")
...
+
 ...
 @app.post("/api/chat")
@@ -444,6 +731,72 @@ async def chat_stream(req: ChatRequest):
